...
V.cofre = async (el) => {
  ...
  el.innerHTML = `
    <div class="cabecalho"><div><h1>Cofre de habilitação</h1><p>Documentos de ${esc(empresaAtual().razao_social)}, usados para conferir cada edital.</p></div>
      <button class="botao" id="novo-doc">Novo documento</button></div>
    ${guia(`<p>Mantenha aqui os documentos de habilitação com a validade atualizada. Na análise de cada edital, o Certame
...
function linhaDocumento(d) {
  ...
  return `<div class="lista-item"><div class="corpo"><b>${esc(d.tipo)}</b>
    ...
    <div class="acoes">${carimbo(sit[0], sit[1])}
      ${d.tem_arquivo ? `<button class="botao texto pequeno" data-baixar-doc="${d.id}" data-nome="${esc(d.nome_arquivo)}">Baixar</button>` : ""}
      <button class="botao secundario pequeno" data-editar-doc="${d.id}">Editar</button>
      <button class="botao texto pequeno" data-excluir-doc="${d.id}" aria-label="Excluir">Excluir</button></div></div>`;
}
